...
This ZSH bash script adds additional functionality to how you use Git. If you frequently work with long branch names that include issue numbers, this tool can be very helpful. With this tool, you'll be able to assign alias names for each branch you create, and switch or delete them based on the alias, instead of the long and confusing name.
...
*  branchAlias \<id> \<alias> [\<note>] : Adds alias and note to a branch that is not stored yet
...
to add the functionalities, added `source gitBranchAlias.sh` in zsh bash (usually `~/.zshrc`). 
